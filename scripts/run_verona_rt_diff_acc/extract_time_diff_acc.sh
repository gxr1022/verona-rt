    ...
    if [ -d "$sub_dir" ]; then
        ...
        for logfile in "$sub_dir"/*.log; do
            if [ -f "$logfile" ]; then
                ...
                # time_value=$(grep "Schedule behaviours time:" "$logfile" | awk -F':' '{print $2}')
                # time_value=$(grep "Init scheduler time:" "$logfile" | awk '{print $4}')
                time_value=$(grep "Run behaviours time:" "$logfile" | awk  '{print $4}')
