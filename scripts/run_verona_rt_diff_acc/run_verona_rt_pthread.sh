...
for acc in ${accounts[*]}; do
    ...
    for t in ${threads[*]}; do
        for tn in ${test_name[*]}; do
            ...
            timeout -v 3600 stdbuf -o0 ${cmd} --pthread --cores ${t} --work_usec ${work} --num_trans ${trans} --accounts ${acc} 2>&1 | tee -a ${this_log_path}
...
python3 ${RUN_PATH}/scripts/run_verona_rt_diff_acc/plot_time_diff_acc.py ${current}
